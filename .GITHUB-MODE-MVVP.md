...
This document defines the **minimum viable valuable product (MVVP)** for OpenClaw GitHub Mode.

It builds on `.GITHUB-MODE-MVP.md` by adding one critical bar: the first release must not only be shippable, it must produce **repeated, measurable value** for maintainers and teams.
...
2. **Usefulness:** maintainers save real time in review and change preparation.
...
MVVP answers “is it already worth keeping on by default?”

---

## 2) Value-Critical User Jobs

The MVVP is scoped around the highest-value jobs from the GitHub Mode docs:
...
If a capability does not improve one of these jobs, it is out of MVVP scope.
...
## 4) MVVP Value Metrics (Required)

MVVP must be measured, not asserted. A weekly rollup must report:
...
4. Value metrics show repeat usage and net maintainer benefit.
