...
### 5.1 Multi-entity cloning and templating contract

GitHub mode must support users creating multiple OpenClaw entities inside their own GitHub account (user/org) with repeatable scaffolding:

- template bootstrap source:
  - GitHub Template Repository flow
  - optional bootstrap workflow dispatch for existing repos
- each entity repo must include:
  - `runtime/github/entity-manifest.json` (entity id, role, trust, communication permissions)
  - `runtime/github/collaboration-policy.json` (which entities can talk to which entities and by what channel)
  - `runtime/github/remote-entities.json` (address book of peer entities)
- repository-level onboarding automation should configure:
  - default labels, issue forms, PR templates, CODEOWNERS, branch protections
  - required environments and secrets/variables placeholders
  - baseline workflows enabled in a known-good state

Template compatibility is a first-class requirement. Any change to GitHub mode contracts must include template migration notes so newly cloned entities and existing entities remain interoperable.

### 5.2 Collaboration and communication contract

Multiple OpenClaw entities created from templates must communicate through explicit, auditable GitHub-native mechanisms:

- `repository_dispatch` for event-to-event signaling across repos
- `workflow_call` for reusable, versioned cross-entity orchestration
- issues/PR comments for human-visible collaboration trails
- discussions for long-running design deliberation where appropriate
- artifacts + attestations as immutable evidence shared between entities

Every inter-entity message must include correlation metadata:

- source entity id
- target entity id
- command/intent type
- source commit SHA and workflow run id
- trust level and policy version
- timestamp and optional expiry/ttl

Collaboration must be deny-by-default: only declared routes in `collaboration-policy.json` are permitted.
...
### 6.4 Multi-entity bootstrap and collaboration workflows

- `github-mode-bootstrap-entity.yml`
  - initializes or validates a repository created from template
  - sets up labels, issue forms, PR templates, and environment placeholders
- `github-mode-sync-templates.yml`
  - verifies template drift and applies migration guidance
- `github-mode-collab-dispatch.yml`
  - sends policy-checked `repository_dispatch` events to peer entities
- `github-mode-collab-receive.yml`
  - validates inbound collaboration events and routes to safe command handlers
...
### Phase 5: Multi-entity GitHub collaboration maturity

- ship entity template bootstrap and migration flow
- enforce collaboration policies and correlation metadata on cross-entity workflows
- add reusable orchestration workflows (`workflow_call`) for entity-to-entity coordination
- expose collaboration health metrics and incident hooks
...
6. A user can clone/template multiple OpenClaw entities inside one GitHub account and have them collaborate using policy-governed GitHub-native communication paths.
