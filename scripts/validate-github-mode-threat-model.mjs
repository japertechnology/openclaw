#!/usr/bin/env node

import Ajv2020 from "ajv/dist/2020.js";
import { readFileSync } from "node:fs";
import { resolve } from "node:path";

const schemaPath = resolve("runtime/github/threat-model/trigger-matrix.schema.json");
const matrixPath = resolve("runtime/github/threat-model/trigger-matrix.json");

const schema = JSON.parse(readFileSync(schemaPath, "utf8"));
const matrix = JSON.parse(readFileSync(matrixPath, "utf8"));

const requiredTriggerVariants = [
  ["pull_request", "fork"],
  ["pull_request", "internal"],
  ["push", "protected_branch"],
  ["schedule", "default"],
  ["workflow_dispatch", "default"],
];

const requiredSecurityGoals = [
  "no_privileged_execution_in_untrusted_contexts",
  "privileged_branch_mutation_requires_pr_flow",
];

const errors = [];

const ajv = new Ajv2020({ allErrors: true, strict: false });
const validate = ajv.compile(schema);
const validAgainstSchema = validate(matrix);

if (!validAgainstSchema) {
  for (const issue of validate.errors ?? []) {
    const pointer = issue.instancePath || "/";
    const message = issue.message ?? "schema validation error";
    errors.push(`schema ${pointer}: ${message}`);
  }
}

for (const goal of requiredSecurityGoals) {
  if (!Array.isArray(matrix.securityGoals) || !matrix.securityGoals.includes(goal)) {
    errors.push(`missing required security goal: ${goal}`);
  }
}

if (!Array.isArray(matrix.contexts)) {
  errors.push("contexts must be an array");
} else {
  const seenContexts = new Set();

  for (const [trigger, variant] of requiredTriggerVariants) {
    const found = matrix.contexts.some(
      (context) => context.trigger === trigger && context.variant === variant,
    );
    if (!found) {
      errors.push(`missing required context: ${trigger}/${variant}`);
    }
  }

  for (const context of matrix.contexts) {
    const contextKey = `${context.trigger}/${context.variant}`;
    if (seenContexts.has(contextKey)) {
      errors.push(`duplicate context entry: ${contextKey}`);
    }
    seenContexts.add(contextKey);

    if (context.trustLevel === "untrusted" && context.privilegedExecutionAllowed === true) {
      errors.push(
        `guardrail violation: untrusted context ${contextKey} allows privileged execution`,
      );
    }

    if (context.privilegedExecutionAllowed === true && context.branchMutationPolicy !== "pr_only") {
      errors.push(
        `guardrail violation: privileged context ${contextKey} must enforce branchMutationPolicy=pr_only`,
      );
    }
  }
}

if (!Array.isArray(matrix.abuseCases) || matrix.abuseCases.length === 0) {
  errors.push("abuseCases must be a non-empty array");
} else {
  const seenAbuseCaseIds = new Set();

  for (const abuseCase of matrix.abuseCases) {
    const abuseCaseId = abuseCase.id ?? "<unknown>";

    if (seenAbuseCaseIds.has(abuseCaseId)) {
      errors.push(`duplicate abuse case id: ${abuseCaseId}`);
    }
    seenAbuseCaseIds.add(abuseCaseId);

    if (!Array.isArray(abuseCase.preventiveControls) || abuseCase.preventiveControls.length === 0) {
      errors.push(`abuse case ${abuseCaseId} is missing preventive controls`);
    }
    if (!Array.isArray(abuseCase.detectiveControls) || abuseCase.detectiveControls.length === 0) {
      errors.push(`abuse case ${abuseCaseId} is missing detective controls`);
    }
  }
}

if (errors.length > 0) {
  console.error("GitHub mode threat model validation failed:");
  for (const error of errors) {
    console.error(`- ${error}`);
  }
  process.exit(1);
}

console.log("GitHub mode threat model validation passed.");
